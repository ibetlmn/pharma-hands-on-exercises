...
author: "Yvette Lorraine Novicio"
date: "02/24/2025"
...
```{r}
varSummary <- function(.data = NULL, parameter = NULL) {
    .data %>%
    filter(SAFFL=="Y" & VISIT=="SCREENING" & PARAMCD=={{parameter}}) %>%
    mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
    group_by(PARAM,TRTAN,TRTA,SEX) %>%
    summarize(mean=mean(AVAL) %>% round(digits=1) %>% format(nsmall=1),
              sd=sd(AVAL) %>% round(digits=1) %>% format(nsmall=1), 
              med=median(AVAL) %>% round(digits=0),
              min=min(AVAL) %>% round(digits=0), 
              max=max(AVAL) %>% round(digits=0),
              n=n()) %>%
    mutate(range_minmax=paste0("(",min, ", ", max, ")")) %>%
    ungroup() %>%
    select("PARAM","TRTA", "SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
    mutate(across(where(is.numeric), .fns=as.character)) %>%
    pivot_longer(-c("PARAM","TRTA","SEX"), names_to="Category", values_to="Values") %>%
...
height<-varSummary(advs, "HGT_CM")
height
weight<-varSummary(advs, "WGHTKG")
weight

final<-rbind(weight, height)
final
```

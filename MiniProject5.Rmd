...
```{r}
advs1 <- import("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdiscpilot01/advs.xpt")

pulse <- advs1 %>% filter(PARAMCD == "PULSE") %>%
  filter(VISITNUM > 3)

plot_pulse <- plot4 %+% pulse

xLab <- quo_name(plot_pulse$mapping$x)
yLab <- quo_name(plot_pulse$mapping$y)
groupLab <- quo_name(plot_pulse$mapping$group)
...
study <- uniqueVal(plot_pulse$data$STUDYID)
measure <- uniqueVal(plot_pulse$data$PARAM)

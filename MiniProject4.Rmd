...
```{r merging data frames for Ethnicity}
...
```{r creating percents for Ethnicity}
ethnic_data_new <- ethnic_mrg_cnt3 %>% 
  mutate(perc_tot_ethnic = round((ethnic_total/N)*100, 1)) %>%                 
  mutate(perc_ethnic = round((ethnic_n/n)*100,1))

ethnic_pct <- ethnic_data_new %>%
  mutate(perc_tchar_ethnic = format(perc_tot_ethnic, nsmall = 1)) %>%
  mutate(perc_achar_ethnic = format(perc_ethnic, nsmall = 1))

ethnic_n_pct <- ethnic_pct %>%
  mutate(npct_ethnic = paste(ethnic_n, paste0("(", perc_achar_ethnic, ")"))) %>% 
  select(ETHNIC, TRT01A, SEX, npct_ethnic)
```

```{r ethnic_cat}
ethnic_trans <- pivot_wider(ethnic_n_pct, 
                         names_from = c(TRT01A,SEX), 
                         values_from = npct_ethnic, 
                         values_fill = "0",
                         names_sep = "_")
...
```{r merging data frames for Race}
...
```{r creating percents for Race}
race_data_new <- race_mrg_cnt3 %>% 
  mutate(perc_tot_race = round((race_total/N)*100, 1)) %>%                 
  mutate(perc_race = round((race_n/n)*100,1))

race_pct <- race_data_new %>%
  mutate(perc_tchar_race = format(perc_tot_race, nsmall = 1)) %>%
  mutate(perc_achar_race = format(perc_race, nsmall = 1))

race_n_pct <- race_pct %>%
  mutate(npct_race = paste(race_n, paste0("(", perc_achar_race, ")"))) %>% 
  select(RACE, TRT01A, SEX, npct_race)
```

```{r race_cat}
race_trans <- pivot_wider(race_n_pct, 
                         names_from = c(TRT01A,SEX), 
                         values_from = npct_race, 
                         values_fill = "0",
                         names_sep = "_")
...
```{r Final Demographic Table}
dm_allcomb3 <- rbind("", dm_allcomb2)
dm_allcomb3[1,1] <- "Age Category, n (%)"
race_cat[1,1] <- "Race, n (%)"
race_cat <- rbind("", race_cat)
ethnic_cat[1,1] <- "Ethnicity, n (%)"
ethnic_cat <- rbind("", ethnic_cat)
final <- bind_rows(dm_allcomb3, ethnic_cat, race_cat)
final
```

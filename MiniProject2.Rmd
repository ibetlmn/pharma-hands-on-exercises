...
```{r small n - Challenge 2}
small_n_cnt2 <- adae_saf %>%
  group_by( TRTAN, TRTA, AEBODSYS) %>%
  count(name = "n")
small_n_cnt2
```

```{r left join - Challenge 2}
adae_mrg_cnt <- small_n_cnt2 %>%
...
```{r ungroup - Challenge 2}
adae_mrg_cnt %>%
  ungroup()
```

```{r pipeline - Challenge 2}
# Paste commands above after the "<-" assignment, one per line
adae_mrg_cnt <- small_n_cnt2 %>%
  left_join(Big_N_cnt2) %>%
  mutate(perc = round(n/N*100, digits=1)) %>%
